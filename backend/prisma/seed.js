...
function lastSchoolDays(refDate, count) {
    const days = [];
...
async function main() {
    ...
    const [aulaAM, aulaAT, aulaBM, aulaBT, aulaCM, aulaCT] = await Promise.all([
        prisma.aula.create({ data: { cicloId: ciclo.id, nombre: 'C-001', turno: client_1.Turno.manana, area: client_1.Area.ciencias, cupoMaximo: 40 } }),
        prisma.aula.create({ data: { cicloId: ciclo.id, nombre: 'C-002', turno: client_1.Turno.tarde, area: client_1.Area.ciencias, cupoMaximo: 40 } }),
        prisma.aula.create({ data: { cicloId: ciclo.id, nombre: 'L-001', turno: client_1.Turno.manana, area: client_1.Area.letras, cupoMaximo: 40 } }),
        prisma.aula.create({ data: { cicloId: ciclo.id, nombre: 'L-002', turno: client_1.Turno.tarde, area: client_1.Area.letras, cupoMaximo: 40 } }),
        prisma.aula.create({ data: { cicloId: ciclo.id, nombre: 'M-001', turno: client_1.Turno.manana, area: client_1.Area.medicas, cupoMaximo: 35 } }),
        prisma.aula.create({ data: { cicloId: ciclo.id, nombre: 'M-002', turno: client_1.Turno.tarde, area: client_1.Area.medicas, cupoMaximo: 35 } }),
    ]);
    ...
    const [carCivil, carSistemas, carAgro, carDerecho, carEconomia, carPoliticas, carEnfermeria, carObstetricia, carFarmacia] = await Promise.all([
        prisma.carrera.create({ data: { nombre: 'Ing. Civil', area: client_1.Area.ciencias } }),
        prisma.carrera.create({ data: { nombre: 'Ing. de Sistemas', area: client_1.Area.ciencias } }),
        prisma.carrera.create({ data: { nombre: 'Ing. Agronómica', area: client_1.Area.ciencias } }),
        prisma.carrera.create({ data: { nombre: 'Derecho', area: client_1.Area.letras } }),
        prisma.carrera.create({ data: { nombre: 'Economía', area: client_1.Area.letras } }),
        prisma.carrera.create({ data: { nombre: 'Ciencias Políticas', area: client_1.Area.letras } }),
        prisma.carrera.create({ data: { nombre: 'Enfermería', area: client_1.Area.medicas } }),
        prisma.carrera.create({ data: { nombre: 'Obstetricia', area: client_1.Area.medicas } }),
        prisma.carrera.create({ data: { nombre: 'Farmacia', area: client_1.Area.medicas } }),
    ]);
    console.log('   ✓ Carreras (9): 3 × área');
    const carrerasPorArea = {
        [client_1.Area.ciencias]: [carCivil, carSistemas, carAgro],
        [client_1.Area.letras]: [carDerecho, carEconomia, carPoliticas],
        [client_1.Area.medicas]: [carEnfermeria, carObstetricia, carFarmacia],
    };
    ...
    const docentes = await Promise.all(docentesDef.map((d) => prisma.docente.create({
        data: {
            dni: d.dni, nombre: d.nombre, apellidos: d.apellidos,
            especialidad: d.especialidad, telefonoWhatsapp: d.tel,
            usuario: { create: { email: d.email, passwordHash: hashDocente, rol: client_1.Rol.docente } },
        },
    })));
    ...
    const alumnos = await Promise.all(alumnosDef.map((a, i) => {
    ...
    const apoderados = await Promise.all(apoderadosDef.map((ap) => prisma.apoderado.create({
        data: {
            dni: ap.dni, nombre: ap.nombre, apellidos: ap.apellidos, telefonoWhatsapp: ap.tel,
            usuario: { create: { email: ap.email, passwordHash: hashApoderado, rol: client_1.Rol.apoderado } },
        },
    })));
    await prisma.alumnoApoderado.createMany({
        data: apoderadosDef.flatMap((ap, apIdx) => ap.hijoIdxs.map((alumnoIdx, pos) => ({
    ...
    const horariosData = aulaConfigs.flatMap(({ aula, baseH, offset }) => slotPattern.map((s, slotIdx) => {
    ...
    for (let ai = 0; ai < alumnos.length; ai++) {
        ...
        for (let di = 0; di < schoolDays.length; di++) {
            ...
            alumnoAsist.push({
                tipoPersona: client_1.TipoPersona.alumno,
    ...
    for (let di = 0; di < docentes.length; di++) {
        for (let dayIdx = 0; dayIdx < schoolDays.length; dayIdx++) {
            ...
            docenteAsist.push({
                tipoPersona: client_1.TipoPersona.docente,
    ...
    const comunicadosDef = [
        {
            ...
            destinatarioTipo: client_1.TipoDestinatario.todos,
            canalWhatsapp: true,
            aulaId: null,
            daysAgo: 12,
        },
        {
            ...
            destinatarioTipo: client_1.TipoDestinatario.alumnos,
            canalWhatsapp: false,
            aulaId: null,
            daysAgo: 9,
        },
        {
            ...
            destinatarioTipo: client_1.TipoDestinatario.seccion,
            canalWhatsapp: true,
            aulaId: aulaAM.id,
            daysAgo: 6,
        },
        {
            ...
            destinatarioTipo: client_1.TipoDestinatario.apoderados,
            canalWhatsapp: true,
            aulaId: null,
            daysAgo: 4,
        },
    ...
    await Promise.all(comunicadosDef.map((c) => {
    ...
    const recursosDef = [
        { titulo: 'Álgebra: Polinomios y factorización', tipo: client_1.TipoRecurso.pdf, url: 'https://recursos.cepreunasam.edu.pe/mat/algebra-polinomios.pdf', cursoId: cMat.id, nivel: 'Básico', desc: 'Compendio de ejercicios de álgebra con factorización y operaciones con polinomios.' },
        { titulo: 'Aritmética: Números enteros y racionales', tipo: client_1.TipoRecurso.pdf, url: 'https://recursos.cepreunasam.edu.pe/mat/aritmetica-enteros.pdf', cursoId: cMat.id, nivel: 'Básico', desc: 'Guía de repaso de aritmética básica con ejercicios resueltos.' },
        { titulo: 'Física: Cinemática — video explicativo', tipo: client_1.TipoRecurso.video, url: 'https://www.youtube.com/watch?v=ejemplo_fisica_cinematica', cursoId: cFis.id, nivel: 'Intermedio', desc: 'Video de 45 min sobre MRU y MRUA con ejemplos.' },
        { titulo: 'Física: Dinámica — Leyes de Newton', tipo: client_1.TipoRecurso.video, url: 'https://www.youtube.com/watch?v=ejemplo_fisica_newton', cursoId: cFis.id, nivel: 'Intermedio', desc: 'Serie de 3 videos sobre las tres leyes de Newton.' },
        { titulo: 'Tabla periódica interactiva', tipo: client_1.TipoRecurso.enlace, url: 'https://ptable.com/', cursoId: cQui.id, nivel: 'Básico', desc: 'Tabla periódica con propiedades y configuración electrónica.' },
        { titulo: 'Química Orgánica: Grupos funcionales', tipo: client_1.TipoRecurso.pdf, url: 'https://recursos.cepreunasam.edu.pe/qui/organica-grupos-funcionales.pdf', cursoId: cQui.id, nivel: 'Avanzado', desc: 'Resumen de grupos funcionales, reacciones y ejercicios.' },
        { titulo: 'Biología celular — simulador interactivo', tipo: client_1.TipoRecurso.iframe, url: 'https://learn.genetics.utah.edu/content/cells/insideacell/', cursoId: cBio.id, nivel: 'Intermedio', desc: 'Simulador de célula eucariótica con organelas y funciones.' },
        { titulo: 'Lenguaje: Técnicas de comprensión lectora', tipo: client_1.TipoRecurso.pdf, url: 'https://recursos.cepreunasam.edu.pe/len/comprension-lectora.pdf', cursoId: cLen.id, nivel: 'Intermedio', desc: 'Guía práctica con textos modelo para el examen de admisión.' },
        { titulo: 'Gramática: Morfología y sintaxis', tipo: client_1.TipoRecurso.pdf, url: 'https://recursos.cepreunasam.edu.pe/len/gramatica-morfosintaxis.pdf', cursoId: cLen.id, nivel: 'Básico', desc: 'Clases de palabras, análisis morfológico y oraciones.' },
        { titulo: 'Literatura peruana: generaciones y autores', tipo: client_1.TipoRecurso.pdf, url: 'https://recursos.cepreunasam.edu.pe/lit/literatura-peruana.pdf', cursoId: cLit.id, nivel: 'Básico', desc: 'Movimientos literarios peruanos del siglo XX.' },
    ];
